...
// ── Estilos base inspirados no PlacasTab ──────────────────────────────────────
...
const EspacadoresTab = () => {
  ...
  const [espacadores, setEspacadores] = useState([]);
  ...
  useEffect(() => {
    ...
    const subscription = supabase.channel('espacadores-despacho')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'espacadores' }, () => {
        fetchEspacadores();
      })
      .subscribe();
  ...
  const fetchEspacadores = async () => {
    setLoading(true);
    const { data } = await supabase.from('espacadores').select('*').order('dt_criacao', { ascending: false });
    if (data) setEspacadores(data);
    setLoading(false);
  };
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [filtroStatus, filtroLocal, posto]);
  ...
  const locaisUnicos = [...new Set(espacadores.map(e => norm(e.local)).filter(Boolean))].sort();
  ...
  const espacadoresFiltrados = espacadores.filter(e => {
    if (filtroStatus.length > 0 && !filtroStatus.includes(e.status)) return false;
    if (filtroLocal.length > 0 && !filtroLocal.includes(norm(e.local))) return false;
    if (posto && postoDeLocalidade(e.local) !== posto) return false;
    return true;
  });

  const totalPages = Math.max(1, Math.ceil(espacadoresFiltrados.length / PAGE_SIZE));
  const safePage = Math.min(currentPage, totalPages);
  const currentData = espacadoresFiltrados.slice((safePage - 1) * PAGE_SIZE, safePage * PAGE_SIZE);

  const toggleSelectAll = () => {
    if (selectedIds.length === currentData.length && currentData.length > 0) {
      setSelectedIds([]);
    } else {
      setSelectedIds(currentData.map(e => e.id));
    }
  };
  ...
  const handleAtribuir = async () => {
    ...
    try {
      ...
      const promises = selectedIds.map(async (id) => {
        const espacador = espacadores.find(e => e.id === id);
        const novoHist = {
          who: user.label,
          matricula: user.matricula,
          when: new Date().toISOString(),
          msg: `Espaçador atribuído para ${tecnico.nome}`
        };
        
        return supabase.from('espacadores').update({
      ...
      alert('Erro ao atribuir espaçadores.');
    } finally {
  ...
  const handleRemoverAtribuicao = async () => {
    if (!window.confirm(`Tem certeza que deseja remover a atribuição de ${selectedIds.length} espaçadores?`)) return;
  ...
  // Metricas de Resumo
  const totalEspacadores = espacadores.length;
  const concluidos = espacadores.filter(e => e.status === 'concluido').length;
  const pendentes = espacadores.filter(e => e.status === 'pendente').length;
  const comAtribuicao = espacadores.filter(e => e.atribuido_para).length;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px' }}>
        {[
          { label: 'Total de espaçadores',   value: totalEspacadores, color: '#0f2544' },
          { label: 'Atribuídos a técnicos',  value: comAtribuicao,    color: '#1d4ed8' },
          { label: 'Concluídos',             value: concluidos,       color: '#15803d' },
          { label: 'Pendentes',              value: pendentes,        color: '#c2410c' },
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: '16px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div style={{ fontSize: '11px', color: '#64748b', background: '#f1f5f9', borderRadius: '20px', padding: '5px 12px', fontWeight: '600' }}>
              {espacadoresFiltrados.length} encontrados
            </div>
          </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
            <thead style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
              <tr>
                ...
                <th style={{ textAlign: 'left', fontSize: '10px', color: '#94a3b8', fontWeight: '700', padding: '12px', letterSpacing: '0.06em', textTransform: 'uppercase' }}>Data Criação</th>
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan="7" style={{ textAlign: 'center', padding: '40px', color: '#94a3b8' }}>Carregando espaçadores...</td></tr>
              ) : currentData.length === 0 ? (
                <tr><td colSpan="7" style={{ textAlign: 'center', padding: '40px', color: '#94a3b8' }}>Nenhum espaçador encontrado.</td></tr>
              ) : (
                currentData.map((e, i) => {
                  const scfg = STATUS_CONFIG[e.status] || { label: e.status, bg: '#f1f5f9', color: '#475569', border: '#e2e8f0' };
                  const dataFormatada = e.dt_criacao ? new Date(e.dt_criacao).toLocaleDateString('pt-BR') : '—';
                  const isSelected = selectedIds.includes(e.id);
                  
                  return (
                    <tr key={e.id} style={{ borderBottom: '1px solid #f8fafc', background: isSelected ? '#eff6ff' : i % 2 === 0 ? '#fff' : '#fafbfc', transition: 'background 0.1s' }}>
                      <td style={{ padding: '10px 12px', textAlign: 'center', verticalAlign: 'middle' }}>
                        <input type="checkbox" checked={isSelected} onChange={() => toggleSelect(e.id)} style={{ cursor: 'pointer' }} />
                      </td>
                      <td style={{ padding: '10px 12px', fontWeight: '600', color: '#0f2544' }}>{e.id}</td>
                      <td style={{ padding: '10px 12px', color: '#334155' }}>{e.local || '—'}</td>
                      <td style={{ padding: '10px 12px', color: '#334155', fontWeight: '500' }}>{e.equip || '—'}</td>
                      ...
                      <td style={{ padding: '10px 12px' }}>
                        {e.atribuido_para ? (
                          <div style={{ display: 'flex', flexDirection: 'column' }}>
                            <span style={{ color: '#1d4ed8', fontWeight: '600', fontSize: '11px' }}>{e.atribuido_para.nome}</span>
                            {e.atribuido_para.equipe && <span style={{ color: '#94a3b8', fontSize: '10px' }}>{e.atribuido_para.equipe}</span>}
                          </div>
                        ) : (
        ...
        {totalPages > 1 && (
          <div style={{ padding: '12px 16px', borderTop: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#fafbfc' }}>
            <div style={{ fontSize: '11px', color: '#64748b' }}>
              Exibindo <strong style={{ color: '#0f2544' }}>{Math.min((safePage - 1) * PAGE_SIZE + 1, espacadoresFiltrados.length)}–{Math.min(safePage * PAGE_SIZE, espacadoresFiltrados.length)}</strong> de <strong style={{ color: '#0f2544' }}>{espacadoresFiltrados.length}</strong> registros
            </div>
